...
async function handler(req, res) {
    try {
        ...
        const url = `${origin}/api/series?asset=${encodeURIComponent(asset)}${symbol ? `&symbol=${encodeURIComponent(symbol)}` : ""}`;
        ...
        const signal = bullishProb > 0.6 ? "BULLISH" : bearishProb > 0.6 ? "BEARISH" : "NEUTRAL";
        res.status(200).json({
            asset,
            symbol: data.symbol || symbol || null,
            asOf: new Date().toISOString(),
            ...
            note: "Prototype price-only model. Next: blend in news tone, macro calendar, and flows for better accuracy."
        });
    } catch (e) {
        res.status(200).json({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__6c1f8168._.js.map